...
const extractChangedSegment = (
  ...
  // If surrounding context changed, the edit happened outside the
  // highlighted span. Don't treat it as this error being updated.
  return "";
};
...
const ReviewTranslation = () => {
  ...
  const markResolvedErrorsAsChangedForBlock = (
    ...
    chunk.mqm_result.errors = (chunk.mqm_result.errors || []).flatMap(
      (error) => {
        ...
        // If highlighted content is fully removed, remove the issue entirely.
        if (!existsNow && !changedSegment) {
          return [];
        }
  ...
  const handleEditableBlockBlur = (blockIndex, event) => {
    const nextValue = event.currentTarget.innerText;
    if (nextValue !== (translationBlocks[blockIndex]?.text || "")) {
      handleTextBlockChange(blockIndex, nextValue);
    }
  };

  const handleEditableListItemBlur = (blockIndex, itemIndex, event) => {
    const nextValue = event.currentTarget.innerText;
    const currentItems = normalizeListItems(
      translationBlocks[blockIndex]?.text || "",
    );
    if (nextValue !== (currentItems[itemIndex] || "")) {
      handleListItemChange(blockIndex, itemIndex, nextValue);
    }
  };
    ...
    if (manualEdit) {
      if (block.type === "Heading") {
        return (
          <h3
            ...
            onBlur={(event) => handleEditableBlockBlur(index, event)}
            className="rounded-md px-1 text-[28px] font-semibold text-slate-800 outline-none focus:bg-indigo-50/40"
          >
            ...
            {listItems.map((item, itemIndex) => (
              ...
              >
                ...
                <p
                  ...
                  suppressContentEditableWarning
                  onBlur={(event) =>
                    handleEditableListItemBlur(index, itemIndex, event)
                  }
                  className="w-full rounded-md px-1 py-1 text-[18px] leading-8 text-slate-700 outline-none focus:bg-indigo-50/40"
                >
                  {renderHighlightedText(
                    item,
      ...
      return (
        <p
          ...
          suppressContentEditableWarning
          onBlur={(event) => handleEditableBlockBlur(index, event)}
          className="rounded-md px-1 text-[18px] leading-8 text-slate-700 whitespace-pre-wrap outline-none focus:bg-indigo-50/40"
        >
          {renderHighlightedText(
            block.text,
            highlights,
